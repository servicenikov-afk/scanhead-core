...
class StickerGenerator:
    ...
    def generate_simple_preview(self, article, name, quantity=None, unit=None, address=None) -> Image.Image:
        """Упрощенный предпросмотр для ScanHead"""
        w, h = 400, 300
        ...
        y = 20
        draw.text((20, y), f"Артикул: {article}", fill='black', font=font_title)
        y += 35
        draw.text((20, y), f"{name[:60]}", fill='black', font=font_text)
        y += 30
        
        if quantity is not None:
            draw.text((20, y), f"Количество: {quantity} {unit or 'шт'}", fill='black', font=font_text)
            y += 25
        
        if address:
            draw.text((20, y), f"Адрес: {address}", fill='gray', font=font_small)
