...
|----------|-------------|
| Отсутствовало упоминание **гибридного стека ttk+tk** | Добавлен отдельный раздел 2 с критическим предупреждением |
| Структура проекта не соответствовала реальной | Полностью переписан раздел 6 под актуальную структуру из `current_structure.MD` |
| Не указано про read-only БД и потокобезопасность | Добавлено в раздел 5 |
| Не указаны адаптеры как отдельный слой | Добавлен слой "Adapters" в таблицу |
| Запрет на удаление tk не был явным | Добавлен в таблицу табу и в раздел 2 |
| Процесс разработки указывал на несуществующую ветку | Исправлено на `main-gui` → `main` |
