...
### 4. ❌ **gui/services/adapters/store_adapter.py** — Нарушение принципов работы с БД
...
- [ ] ⚠️ **Несогласованность операций**: методы чтения и записи в одном классе без чёткого разделения

**Решения:**
- [ ] Использовать `libs.path_utils.get_db_path()` вместо хрупких путей
...
- [ ] Разделить на `IStoreReader` и `IStoreWriter` для чёткого разделения ответственности
...
### ✅ Решённые баги (5) — ИСКЛЮЧЕНЫ ИЗ TODO
