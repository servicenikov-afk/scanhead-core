...
|---------|-----|----------|--------|
| `article` | TEXT | Артикул (уникальный, PRIMARY KEY) | `566.0000.004` |
| `locations` | TEXT | JSON-массив адресов хранения | `["A-12-3","B-05-2"]` |
...
> ⚠️ **Важно:** Поля `locations` и `alternative_names` хранятся как **TEXT** (строки), содержащие валидный JSON. При чтении необходимо выполнять `json.loads()`.
...
### Получить все адреса артикула (развернуть JSON-массив)
...
### Поиск по альтернативному названию

```sql
SELECT article, alternative_names
FROM storage_locations
WHERE alternative_names LIKE '%Головная%';
```
...
**Связь:** `nomenclature.canonical_article` ↔ `store.article` ↔ `css_export.art_no` (через адаптеры).
...
- **Дата последней правки:** 2026-06-06