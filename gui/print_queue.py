...
class PrintQueue(ctk.CTkFrame):
    _theme_applied = False  # Флаг: тема применена один раз
    ...
    def _apply_treeview_theme(self) -> None:
        """Применить тему clam для Treeview ОДИН РАЗ, после инициализации root."""
    ...
    def _create_table(self) -> None:
        ...
        # Применяем тему только сейчас, когда root уже существует
    ...
    def _delete_item(self, item_id: str) -> None:
        ...
        if 0 <= item_index < len(self._products):
    ...
    def _print_all(self) -> None:
        ...
        for item_id in self._tree.get_children():
            item_index = int(item_id) - 1
            if 0 <= item_index < len(self._products):
                product = self._products[item_index]
                count = self._copy_counts.get(item_id, 1)
