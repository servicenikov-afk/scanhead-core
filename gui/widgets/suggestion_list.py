...
class SuggestionList(ctk.CTkToplevel):
	def __init__(self,master:Any,on_select:Callable[[str],None],width:int=400,max_height:int=200,font_size:int=18,**kwargs):
		...
		self.withdraw()
		self.frame=ctk.CTkScrollableFrame(self,width=0,height=self.max_height,corner_radius=6,border_width=1,scrollbar_button_color="#555555",scrollbar_button_hover_color="#777777")
	...
	def show_suggestions(self,suggestions:List[str],x:int,y:int)->None:
		...
		if not self._is_visible:
			if self.winfo_exists():self.deiconify()
			else:return
	...
	def _rebuild_buttons(self,items:List[str])->None:
		...
		for text in items:
			btn=ctk.CTkButton(self.frame,text=text,anchor="w",height=button_height,command=lambda t=text:self._select(t),hover_color=("gray80","gray20"),font=ctk.CTkFont(size=self._font_size,family="Arial"))
			btn.pack(fill="x",padx=2,pady=1)
			self.buttons.append(btn)
	def _clear_buttons(self)->None:
		for btn in self.buttons:
			if btn.winfo_exists():btn.pack_forget()
		self.buttons.clear()
	...
	def _select(self,value:str)->None:
		...
		if self.on_select:self.on_select(value)
		self.hide()
	def update_font_size(self,font_size:int)->None:
		...
		for btn in self.buttons:
			try:
				if btn.winfo_exists():btn.configure(font=new_font,height=button_height)
	...
	def destroy(self)->None:
		if self._pending_action_id is not None:
			try:self.after_cancel(self._pending_action_id)
			except Exception:pass
			self._pending_action_id=None
		self._clear_buttons()
		try:
