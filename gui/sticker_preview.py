...
class StickerPreview(ctk.CTkFrame):
    ...
    def _init_presets(self):
        if not self._settings_service.get_setting('sticker_presets'):
            default = {
                'sticker': {'width_mm': 60, 'height_mm': 40, 'orientation': 'portrait',
                            'background_color': '#FFFFFF', 'border': False, 'dpi': 300},
                'fonts': {'name_size': 6, 'article_size': 8, 'address_size': 6},
                'layout': {'show_barcode': True, 'show_qr': False,
                           'article_position': 'top', 'show_address': False}
            }
    ...
    def _to_nested(self, flat: dict) -> dict:
        return {
            'sticker': {
                ...
                'background_color': '#FFFFFF',  # Жёстко
    ...
    def _on_editor_close(self):
        try:
            if not self._preview_label.winfo_exists():
                return
        except Exception:
            return
    ...
    def _generate_preview(self):
        ...
        try:
            ...
            # Защита от старого плоского формата (миграция)
            if 'sticker' not in preset:
                preset = self._to_nested(preset)
